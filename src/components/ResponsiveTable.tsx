...
export default function ResponsiveTable({
  ...
  // monta URL de edição SEMPRE com ?table=
  const editUrl = (id: string | number) =>
    `/item/${encodeURIComponent(String(id))}/edit?table=${encodeURIComponent(
      tableName
    )}`;
              ...
              <tr key={it.id} className="text-sm">
                ...
                <td className="px-4 py-2">
                  <div className="flex flex-wrap gap-2">
                    ...
                    {/* Navega com ?table= sempre presente */}
                    <Link
                      to={editUrl(it.id)}
                      className="px-2 py-1 border rounded text-xs bg-white hover:bg-gray-50"
                    >
        ...
        {items.map((it, idx) => (
          <div key={it.id} className="p-4">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              ...
              <Link to={editUrl(it.id)} className="px-2 py-1 border rounded text-xs">
                Editar
              </Link>
        ...
        >
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-4 flex flex-col items-center gap-3">
              {(() => {
                const qrValue = `${window.location.origin}${editUrl(qrItemId!)}`;
                return (
                  <>
